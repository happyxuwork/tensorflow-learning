    ...
    for root, dirs, images in os.walk(path):
        ...
        with tf.Session() as sess:
            ...
            for i in images:
                ...
                img_data = tf.image.resize_images(img_data, [110, 150], method=1)
                for i in range(54):
                    for j in range(40):
                        img_5 = tf.image.crop_to_bounding_box(img_data, j, i, 70, 96)
                        ...
                        with tf.gfile.GFile(
                                                                "E:/programdate/pythonAll/DCGAN-tensorflow-master/data/dls/" + image_name + str(
                                                        i) + str(j) + ".jpg", 'wb') as f:
                            f.write(result.eval())
                print(str(k) + " done...")
            end = time.time()
