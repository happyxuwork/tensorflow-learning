...
'''
@author: xuqiang
tensorflow中两种方式的图像归一化
像归一化使得图像可以抵抗几何变换的攻击，它能够找出图像中的那些不变量，从而得知这些图像原本就是一样的或者一个系列的
...
第一种：将图像的每个灰度值除以255，得到范围为[0,1]的浮点数
第二种：将图像的每个灰度值除以127.5，然后减去1，得到范围为[-1,1]的浮点数----归一化后分布相对稀疏。
'''
