    ...
    if avg_class == None:
        with tf.variable_scope('layer1', reuse=reuse):
            weights = tf.get_variable("weights",
                                      initializer=tf.truncated_normal(shape=[INPUT_NODE, LAYER1_NODE], stddev=0.1))
            biases = tf.get_variable("biases", shape=[LAYER1_NODE], initializer=tf.constant_initializer(0.0))
            layer1 = tf.nn.relu(tf.matmul(input_tensor, weights) + biases)
        with tf.variable_scope('layer2', reuse=reuse):
            weights = tf.get_variable("weights",
                                      initializer=tf.truncated_normal(shape=[LAYER1_NODE, OUTPUT_NODE], stddev=0.1))
        ...
        with tf.variable_scope('layer11', reuse=reuse):
            weights11 = tf.get_variable("weights11",
                                        initializer=tf.truncated_normal(shape=[INPUT_NODE, LAYER1_NODE], stddev=0.1))
            biases11 = tf.get_variable("biases11", shape=[LAYER1_NODE], initializer=tf.constant_initializer(0.0))
            layer11 = tf.nn.relu(tf.matmul(input_tensor, avg_class.average(weights11)) + avg_class.average(biases11))
        with tf.variable_scope('layer21', reuse=reuse):
            weights22 = tf.get_variable("weights22",
                                        initializer=tf.truncated_normal(shape=[LAYER1_NODE, OUTPUT_NODE], stddev=0.1))
            biases22 = tf.get_variable("biases22", shape=[OUTPUT_NODE], initializer=tf.constant_initializer(0.0))
            layer21 = tf.matmul(layer11, avg_class.average(weights22)) + avg_class.average(biases22)
            return layer21
...
def train(mnist):
    ...
    average_y = inference(x, variable_averages, False)
    # @tf.argmax(y_, 1).
